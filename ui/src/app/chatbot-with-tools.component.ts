...
@Component({
  ...
  standalone: true,
  selector: 'app-chatbot-with-tools',
  template: `
    <h2>Chatbot with Tools</h2>
    <p>Talk to the assistant bot (it can use tools like telling time):</p>

    <!-- Chat History Display -->
    <div class="chat-history">
      @for (message of history(); track message; let i = $index) {
        <div class="message" [ngClass]="message.role">
          <strong>{{ message.role === 'user' ? 'You' : 'Assistant Bot' }}:</strong>
          @if (message.role === 'bot') {
...
export class ChatbotWithToolsComponent {
  userMessage: string = '';
